...
def is_netscape_cookie_file(filepath):
    """Check if a cookie file is in Netscape format (not JSON).
    Netscape cookies files must start with '# Netscape HTTP Cookie File'."""
    try:
...
def get_ytdlp_opts(media_type, use_cookies=False, cookie_file=None, cookies_from_browser=None, po_token=None):
    ...
    if cookies_from_browser:
        ...
        if is_netscape_cookie_file(cookie_file):
            opts['cookiefile'] = cookie_file
            print(f"Using cookies from: {cookie_file}")
        else:
            print(f"Warning: Cookie file '{cookie_file}' is not in Netscape format (may be JSON). Skipping cookies — download will proceed without them.")
